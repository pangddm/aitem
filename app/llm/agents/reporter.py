...
8. 【重要】回答中必须包含命令输出的关键数据，不要只给结论。例如：
   ...
   - 查看日志时：给出关键日志片段
   - 检查状态时：给出具体状态值
9. 【最重要】必须直接回答用户的原始问题。如果用户问"集群里有没有出故障的 Pod / 有哪些问题"，
   就要逐条列出具体的故障 Pod（名称、命名空间、状态、故障原因），并说明是否已修复、执行了什么操作、下一步建议。
   不要把与问题无关的信息当作核心结论（例如"你的节点跑的是 k3s 不是独立的 kubelet"这类节点架构描述），
   也不要跑题。如果命令输出没给出所需信息，要明说缺少数据，并说明应执行的正确命令（如 kubectl get pods -A -o wide），
   绝不要编造结果或用无关信息冒充结论。
...
class Reporter(BaseAgent):
    ...
    def _build_context(self, memories: list = None, knowledge_context: str = "", web_context: str = "") -> str:
        ...
        if memories:
            ...
            context_parts.append(
                f"以下是这位用户的历史长期记忆（用户之前明确表达过的偏好、事实或结论，回答时请结合并尊重）：\n"
                f"{chr(10).join(memory_lines)}\n"
                f"规则：这些是用户历史偏好，正常情况下应遵循；与实时工具结果冲突时以实时为准。"
            )
